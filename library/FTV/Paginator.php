<?php
    ...
    class FTV_Paginator
    {
        ...
        /**
         * Create a new FTV_Paginator instance.
         *
         * @param  array  $results
         * @param  int    $page
         * @param  int    $total
         ...
         * @return Paginator
         */
        public static function factory(array $results = array(), $total, $perPage)
        {
            $page = self::page($total, $perPage);

            $last = ceil($total / $perPage);

            return new self($results, $page, $total, $perPage, $last);
        }
        ...
        {
            $page = (strlen(request()->getCrudNumPage())) ? request()->getCrudNumPage() : 1;
        ...
        public function links($adjacent = 3)
        {
            ...
            return '<div class="pagination">' . i::utf8($content) . '</div>';
        }
        ...
        {
            u::event('paginator.previous', closure('$page = $_params->getPage();return $page <= 1;'));

            return $this->element(__function__, $this->page - 1, $text, 'paginator.previous');
        }
        ...
        {
            u::event('paginator.next', closure('$page = $_params->getPage();$last = $_params->getLast(); return $page >= $last;'));

            return $this->element(__function__, $this->page + 1, $text, 'paginator.next');
        }
        ...
        protected function element($element, $page, $text, $disabled)
        {
            ...
            $isDisabled = u::run($disabled, array('page' => $this->page, 'last' => $this->last));
            if ($isDisabled) {
                return '<li' . FTV_Html::attributes(array('class'=>"{$class} disabled")) . '><a href="#">' . i::utf8($text) . '</a></li>';
            } else {
        ...
        public function range($start, $end)
        {
            ...
            for ($page = $start ; $page <= $end ; $page++)  {
                if ($this->page == $page) {
                    $pages[] = '<li class="active"><a href="#" onclick="return false;">' . $page . '</a></li>';
                } else {
        ...
        protected function link($page, $text, $class)
        {
            ...
            return '<li' . FTV_Html::attributes(array('class' => $class)).'><a href="#" onclick="paginationGoPage('.$page.'); return false;">' . i::utf8($text) . '</a></li>';
        }
