<?php
     ...
     */
    class FTV_Image_Adapter_Gd extends FTV_Image_Adapter_Abstract
    {   
        /**
        ...
        */
        ...
        public function __construct($options = array(), $scale = true, $inflate = true, $quality = self::DEFAULT_QUALITY)
        {
            if (!extension_loaded('gd')) {
              throw new FTV_Exception ('GD not enabled. Check your php.ini file.');
            }
        ...
        /**
         * @param $image string
         * @return FTV_Image_Adapter_Gd
         */
        public function open($image)
        {
            $information = @getimagesize($image);
            if (!$information) {
                throw new FTV_Exception(sprintf('Could not load image %s', $image));
            }
            
            if (!in_array($information['mime'], $this->_imgTypes)) {
                throw new FTV_Exception(sprintf('Image MIME type %s not supported', $information['mime']));
            }
            
            $loader = $this->_imgLoaders[$information['mime']];
            if (!function_exists($loader)) {
                throw new FTV_Exception(sprintf('Function %s not available. Please enable the GD extension.', $loader));
            }
            
            $this->_source = $loader($image);
            if (null === $this->_source) {
                throw new FTV_Exception(sprintf('Could not load the file %s', $image));
            }
        ...
        public function load($image, $mime)
        {
            if (!in_array($mime, $this->_imgTypes)) {
                throw new FTV_Exception(sprintf('Image MIME type %s not supported', $mime));
            }
        ...
        public function save($dest, $targetMime = null)
        {
            ...
            if (!file_exists($dirname)) {
                throw new FTV_Exception(sprintf('Directory %s does not exist', $dirname));
            }
            
            if (!is_writable($dirname)) {
                throw new FTV_Exception(sprintf('File %s is not writable', $dirname));
            }
            ...
            if (!function_exists($creator)) {
                throw new FTV_Exception(sprintf('TargetMime %s is not valid', $targetMime));
            }
