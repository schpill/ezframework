<?php
    ...
    abstract class FTV_Cache
    {
         ...
         * @verbatim
         * $myVar = 5;
         * $key   = FTV_Cache::generateKey(12, $myVar, 'foobar', true, 4.45, array(1,2), 'x');
         ...
         *
         * @throws FTV_Exception  if the string contains illegal characters
        ...
        protected function checkString($str, $name)
        {
            if (!is_string($str) && !is_integer($str)) {
                throw new FTV_Exception('Given ' . $name . ' parameter is not a string/int, but a ' . gettype($str) . '.');
            }

            if (is_string($str) && strlen($str) === 0) {
                throw new FTV_Exception('No ' . $name . ' given.');
            }

            if ( $str[0] === '.' || $str[strlen($str)-1] === '.' || strpos($str, '..') !== false || !preg_match('#^[a-z0-9_.\[\]%\#-]+$#i', $str)) {
                throw new FTV_Exception('A malformed ' . $name . ' was given.');
            }
         ...
         *
         * @throws FTV_Exception  if one of the arguments is malformed
