<?php
    ...
    class FTV_Dropbox_Api 
    {
         ...
         * Content API URl
         */
        protected $apiContentUrl = 'https://api-content.dropbox.com/1/';

        /**
         * OAuth object
         *
         * @var FTV_Dropbox_Oauth
         */
        protected $oauth;
         ...
         * Constructor
         *
         * @param Dropbox_OAuth FTV_Dropbox_Oauth object
         * @param string $root default root path (sandbox or dropbox)
         */
        public function __construct(FTV_Dropbox_Oauth $oauth, $root = self::ROOT_DROPBOX, $useSSL = true) 
        {
            $this->oauth = $oauth;
            $this->root = $root;
            $this->useSSL = $useSSL;
            if (!$this->useSSL) {
                throw new FTV_Dropbox_Exception('Dropbox REST API now requires that all requests use SSL.');
            }
        ...
        {
            $data = $this->oauth->fetch($this->apiUrl . 'account/info');
        ...
        public function getFile($path = '', $root = null) {
            ...
            $path = repl(array('%2F', '~'), array('/', '%7E'), rawurlencode($path));
            $result = $this->oauth->fetch($this->apiContentUrl . 'files/' . $root . '/' . ltrim($path, '/'));
            return $result['body'];
        ...
        public function putFile($path, $file, $root = null) {
            ...
            $directory = repl(array('%2F', '~'), array('/', '%7E'), rawurlencode($directory));
            $filename = repl('~', '%7E', rawurlencode($filename));
            ...
            if (is_string($file)) {
                ...
                throw new FTV_Dropbox_Exception('File must be a file-resource or a string');
            }
            
            $result = $this->multipartFetch($this->apiContentUrl . 'files/' . $root . '/' . trim($directory,'/'), $file, $filename);

            if(!isset($result["httpStatus"]) || $result["httpStatus"] != 200) {
                throw new FTV_Dropbox_Exception("Uploading file to Dropbox failed");
            }
        ...
        public function copy($from, $to, $root = null) 
        {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'fileops/copy', array('from_path' => $from, 'to_path' => $to, 'root' => $root), 'POST');
        ...
        public function createFolder($path, $root = null) {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'fileops/create_folder', array('path' => $path, 'root' => $root), 'POST');
            return json_decode($response['body'], true);
        ...
        public function delete($path, $root = null) {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'fileops/delete', array('path' => $path, 'root' => $root), 'POST');
            return json_decode($response['body'], true);
        ...
        public function move($from, $to, $root = null) 
        {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'fileops/move', array('from_path' => rawurldecode($from), 'to_path' => rawurldecode($to), 'root' => $root), 'POST');
        ...
        public function getMetaData($path, $list = true, $hash = null, $fileLimit = null, $root = null) 
        {
            ...
            $path = repl(array('%2F', '~'), array('/', '%7E'), rawurlencode($path));
            $response = $this->oauth->fetch($this->apiUrl . 'metadata/' . $root . '/' . ltrim($path, '/'), $args);
        ...
        */
        public function delta($cursor) {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'delta', $arg, 'POST');
            return json_decode($response['body'], true);
        ...
        public function getThumbnail($path, $size = 'small', $root = null) 
        {
            ...
            $path = repl(array('%2F','~'), array('/','%7E'), rawurlencode($path));
            $response = $this->oauth->fetch($this->apiContentUrl . 'thumbnails/' . $root . '/' . ltrim($path, '/'), array('size' => $size));
        ...
        public function search($query = '', $root = null, $path = '')
        {
            ...
            $response = $this->oauth->fetch($this->apiUrl . 'search/' . $root . '/' . ltrim($path,'/'),array('query' => $query));
            return json_decode($response['body'], true);
        }
        ...
        public function share($path, $root = null) 
        {
            ...
            $path = repl(array('%2F','~'), array('/','%7E'), rawurlencode($path));
            $response = $this->oauth->fetch($this->apiUrl .  'shares/'. $root . '/' . ltrim($path, '/'), array(), 'POST');
            return json_decode($response['body'], true);
        ...
        */
        public function media($path, $root = null) 
        {
            ...
            $path = repl(array('%2F','~'), array('/','%7E'), rawurlencode($path));
            $response = $this->oauth->fetch($this->apiUrl .  'media/'. $root . '/' . ltrim($path, '/'), array(), 'POST');
            return json_decode($response['body'], true);
        ...
        */
        public function copy_ref($path, $root = null) 
        {
            ...
            $path = repl(array('%2F','~'), array('/','%7E'), rawurlencode($path));
            $response = $this->oauth->fetch($this->apiUrl .  'copy_ref/'. $root . '/' . ltrim($path, '/'));
            return json_decode($response['body'], true);
