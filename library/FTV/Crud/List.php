<?php
     ...
     * @package         FTV
     * @subpackage      Crud
     * @see             ?p=crud&ajf_entity=metanavusage&ajf_table=videoevent&action=list
    ...
    class FTV_Crud_List
    {
        ...
        public function __construct($em)
        {
            ...
            $this->_config  = config::get('crud.' . get_class($em) . '.info');
            if (null === $this->_config) {
                $this->_config = FTV_Crud::defaultConfig($em);
            }
        }
        ...
        public function select()
        {
            ...
            $where          = (!strlen($this->_request->getCrudWhere())) ? '' : FTV_Crud::makeQuery($this->_request->getCrudWhere(), $this->_em);
            ...
            if (true === $pagination) {
                ...
                $paginator         = FTV_Paginator::factory((array) $data, $count, $limit);
                $this->_items      = $paginator->getItemsByPage($pageNumber);
                $this->_pagination = FTV_Crud::pagination($paginator);
            } else {
                $this->_items   = $data;
            }

            if (0 < $count && null !== $export) {
                $method = 'export' . ucfirst(i::lower($export));
                return FTV_Crud::$method($data, $this->_em);
            }
        ...
        public function render()
        {
            ...
            $html = '<table class="table table-striped ' . $sorted . ' table-bordered">' . NL;
            $html .= '<thead>' . NL;
            $html .= '<tr>' . NL;
            foreach ($fields as $field => $infosField) {
                if (true === $infosField['onList']) {
                    if (true !== $infosField['sortable']) {
                        $html .= '<th class="no-sorter">' . FTVHelper_Html::display($infosField['label']) . '</th>' . NL;
                    } else {
                        if ($field == $order) {
                            $directionJs = ('ASC' == $orderDirection) ? 'DESC' : 'ASC';
                            $js = 'orderGoPage(\'' . $field . '\', \'' . $directionJs . '\');';
                            $html .= '<th>
                                <div onclick="' . $js . '" class="text-left field-sorting ' . i::lower($orderDirection) . '" rel="' . $field . '">
                                ' . FTVHelper_Html::display($infosField['label']) . '
                                </div>
                            </th>';
                        } else {
            ...
            foreach ($this->_items as $item) {
                ...
                foreach ($fields as $field => $infosField) {
                    if (true === $infosField['onList']) {
                        ...
                        $html .= '<td>'. FTVHelper_Html::display($value) . '</td>' . NL;
                    }
                }
                ...
                if (true === $viewable) {
                    $actions .= '<a href="'. u::getRoute('CRUD_view', array('id' => $item->getId(), 'ajf_entity' => $item->_getEntity(), 'ajf_table' => $item->_getTable())) . '"><i title="afficher" class="icon-file"></i></a>&nbsp;&nbsp;&nbsp;';
                }

                if (true === $editable) {
                    $actions .= '<a href="'. u::getRoute('CRUD_edit', array('id' => $item->getId(), 'ajf_entity' => $item->_getEntity(), 'ajf_table' => $item->_getTable())) . '"><i title="éditer" class="icon-edit"></i></a>&nbsp;&nbsp;&nbsp;';
                }

                if (true === $duplicable) {
                    $actions .= '<a href="'. u::getRoute('CRUD_duplicate', array('id' => $item->getId(), 'ajf_entity' => $item->_getEntity(), 'ajf_table' => $item->_getTable())) . '"><i title="dupliquer" class="icon-plus"></i></a>&nbsp;&nbsp;&nbsp;';
                }

                if (true === $deletable) {
                    $actions .= '<a href="#" onclick="if (confirm(\'Confirmez-vous la suppression de cet élément ?\')) document.location.href = \''. u::getRoute('CRUD_delete', array('id' => $item->getId(), 'ajf_entity' => $item->_getEntity(), 'ajf_table' => $item->_getTable())) . '\';"><i title="supprimer" class="icon-trash"></i></a>&nbsp;&nbsp;&nbsp;';
                }
        ...
        {
            $where = (!strlen($this->_request->getCrudWhere())) ? '' : FTV_Crud::makeQueryDisplay($this->_request->getCrudWhere(), $this->_em);
            ...
            foreach ($this->_config['fields'] as $field => $infosField) {
                if (true === $infosField['searchable']) {
                    ...
                    $search .= '<label class="control-label">' . FTVHelper_Html::display($infosField['label']) . '</label>' . NL;
                    ...
                    if (empty($content)) {
                        ...
                        $search  .= FTV_Crud::internalFunction($content);
                    }
