<?php
     ...
     * placed. Because of the restriction in FTV_Cache::checkString(), you cannot
     ...
     * Before using it, consider using %FTV_CacheFilesystem instead, which
     * should suffice in 99,9% of all cases (if you really need to store large
     * amount of data, consider using Memcached instead).
     *
     * @author  Gerald
     */
    class FTV_CacheFilesystem extends FTV_Cache implements FTV_CacheInterface
    {
            ...
            public function flush($namespace, $recursive = false)
            {
                ...
                $root      = $this->dataDir . DS . $namespace;
                if ($recursive) {
                    ...
                    return $this->deleteRecursive($root . DS . $dataDir);
                }
            }
            ...
            protected function createNamespaceDir($namespace, $root, $hash)
            {
                if (!empty($namespace)) {
                    ...
                    $dir      = $root . DS . $thisPart;

                    $this->makeDir($dir);
                    return $this->createNamespaceDir($namespace, $dir, $hash);
                } else {
                    $dir = $root . DS .'data' . self::$safeDirChar;
                    $this->makeDir($dir);
            ...
            protected function dataDirExists($namespace, $root)
            {
                $namespace = $this->getDirFromNamespace($namespace);
                $dataDir   = 'data' . self::$safeDirChar;
                $dirname   = $root . DS . $namespace . DS . $dataDir;
            ...
            protected function getFilename($namespace, $key)
            {
                ...
                $dir       = $dir . DS . $namespace . DS . $dataDir;
             ...
             *
             * @throws FTV_CacheException  if the directory could not be created
            ...
            protected function makeDir($dir)
            {
                if (!is_dir($dir)) {
                    if (!@mkdir($dir, self::$dirPerm, true)) {
                        throw new FTV_Exception('Can\'t create directory in ' . $dir . '.');
                    }
