<?php
    class FTV_Html_Pdf
    {
        ...
        public function __construct(array $options = array())
        {
            ...
            if (!ake('path', $options)) {
                throw new FTV_Exception("Path to directory where to store files is not set.");
            }

            if (!is_writable($options['path'])) {
                throw new FTV_Exception("Path to directory where to store files is not writable.");
            }
        ...
        protected function _createFile()
        {
            ...
            } while(FTV_File::exists($this->_filename));

            /**
             * create an empty file
             */
            FTV_File::put($this->_filename, $this->getHtml());
        ...
        public function setPath($path)
        {
            if (realpath($path) === false) {
                throw new FTV_Exception("Path must be absolute.");
            }
        ...
        protected function _render()
        {
            if (mb_strlen($this->_html, 'utf-8') === 0 && empty($this->_url))
                throw new FTV_Exception("HTML content or source URL not set.");

            if ($this->getUrl()) {
                $input = $this->getUrl();
            } else {
                FTV_File::put($this->getFilePath(), $this->getHtml());
            ...
            if (strpos(i::lower($content['stderr']), 'error'))
                    throw new FTV_Exception("System error <pre>" . $content['stderr'] . "</pre>");

            if (mb_strlen($content['stdout'], 'utf-8') === 0)
                   throw new FTV_Exception("WKHTMLTOPDF didn't return any data.");

            if ((int)$content['return'] > 1)
                throw new FTV_Exception("Shell error, return code : " . (int)$content['return']);
            ...
            switch ($mode) {
                case self::MODE_DOWNLOAD:
                    if (!headers_sent()) {
                        ...
                        throw new FTV_Exception("Headers already sent.");
                    }
                    break;
                ...
                case self::MODE_EMBEDDED:
                    ...
                    break;
                case self::MODE_SAVE:
                    FTV_File::put($this->getPath() . $filename, $this->_render());
                    $filepath = $this->getFilePath();
                    if (!empty($filepath)) {
                        unlink($filepath);
                    }
                    break;
                default:
                    throw new FTV_Exception("Mode: " . $mode . " is not supported.");
            }
        }
    }
