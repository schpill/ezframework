<?php
    ...
    class FTV_Html_Twitter
    {
        public static function code($code, $isPreScrollable = false, array $attributes = array())
        {
            ...
            return sprintf('<pre%s>%s</pre>', FTV_Html::attributes($attributes), FTV_Html::escape($code));
        }
        ...
        public static function blockquote($text, $source = null, $cite = null, $citeTitle = null, array $attributes = array())
        {
            if (null !== $source) {
                $citeStr = null;
                if (null !== $cite) {
                    $citeStr = sprintf('<cite title="%s">%s</cite>', FTV_Html::escape($citeTitle), FTV_Html::escape($cite));
                }

                $source = sprintf(' <small>%s</small>', FTV_Html::escape($source) . ' ' . $citeStr);
            }

            return sprintf('<blockquote%s>%s</blockquote>', (count($attributes) ? FTV_Html::attributes($attributes) : ''), FTV_Html::escape($text) . $source);
        }
        ...
        public static function lead($text, array $attributes = array())
        {
            ...
            return sprintf('<p%s>%s</p>', FTV_Html::attributes($attributes), FTV_Html::escape($text));
        }

        public static function circledImage($src, array $attributes = array())
        {
            return self::_renderImage($src, 'img-circle', $attributes);
        }

        public static function roundedImage($src, array $attributes = array())
        {
            return self::_renderImage($src, 'img-rounded', $attributes);
        }

        public static function polaroidImage($src, array $attributes = array())
        {
            return self::_renderImage($src, 'img-polaroid', $attributes);
        }
        ...
        */
        protected static function _renderImage($src, $className, array $attributes = array())
        {
            ...
            return sprintf('<img src="%s"%s />', FTV_Html::escape($src), FTV_Html::attributes($attributes));
        }
    }
