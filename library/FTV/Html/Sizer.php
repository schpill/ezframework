<?php
    ...
    class FTV_Html_Sizer 
    {
        ...
        function __construct($file)
        {
            ...
            FTV_Config::add('sizer');
            $this->config = FTV_Config::get('sizer');
            
            // Open up the file.
            $this->image = $this->openImage($file);
            
            if (!$this->image) {
                throw new FTV_Exception('File not recognised. Possibly because the path is wrong. Keep in mind, paths are relative to the main index.php file.');
            }
         ...
         * @param  mixed $file The file array provided by a path to a file
         * @return FTV_Html_Sizer
         */
        public static function open($file)
        {
            return new FTV_Html_Sizer($file);
        }
        ...
        public function save($savePath, $imageQuality = 95)
        {
            ...
            $extension = i::lower(FTV_File::extension($savePath));
        ...
        private function openImage($file)
        {
            ...
            if (!is_array($file)) {
                ...
                $file = array(
                    'type'      => i::lower($file_dimensions['mime']),
                    'tmp_name'  => $file
               );
            }
            ...
            if (!file_exists($file_path)) {
                throw new FTV_Exception('Could not find file: ' . $file_path . '. It doesn\'t seem to exist.');
            }

            switch ($mime)
            {
                case 'image/pjpeg': // IE6
                case FTV_File::mime('jpg'): $img = @imagecreatefromjpeg($file_path);  break;
                case FTV_File::mime('gif'): $img = @imagecreatefromgif($file_path);   break;
                case FTV_File::mime('png'): $img = @imagecreatefrompng($file_path);   break;
        ...
        private function getCropPoints($optimalWidth, $optimalHeight, $newWidth, $newHeight) 
        {
            ...
            switch ($vertical_start) {
                ...
                default:
                    throw new FTV_Exception('Unknown value for crop_vertical_start_point: '. $vertical_start .'. Please check config file in the Resizer bundle.');
                    break;
            }
            ...
            switch ($horizontal_start) {
                ...
                default:
                    throw new FTV_Exception('Unknown value for crop_horizontal_start_point: '. $horizontal_start .'. Please check config file in the Resizer bundle.');
                    break;
            }
