<?php
     ...
     * @author  Gerald
     */
    class FTV_CacheSQLite extends FTV_Cache implements FTV_CacheInterface
    {
            ...
            public function __construct(PDO $connection)
            {
                ...
                $stmt   = $this->pdo->query('SELECT * FROM sqlite_master WHERE type = "table" AND name = "FTV_Cache"');
                $tables = $stmt->fetchAll();

                if (empty($tables)) {
                    $sql = 'CREATE TABLE "FTV_Cache" ("namespace" VARCHAR(255), "keyhash" VARCHAR(40), "payload" BLOB, PRIMARY KEY ("namespace", "keyhash"))';
                    $this->pdo->exec($sql);
                }
            ...
            protected function getStatement($key)
            {
                ...
                $queries = array(
                    'insert'  => 'INSERT OR IGNORE INTO "FTV_Cache" ("namespace", "keyhash", "payload") VALUES (:namespace,:hash,:payload)',
                    'replace' => 'INSERT OR REPLACE INTO "FTV_Cache" ("namespace", "keyhash", "payload") VALUES (:namespace,:hash,:payload)',
                    'select'  => 'SELECT "payload" FROM "FTV_Cache" WHERE ' . $where,
                    'delete'  => 'DELETE FROM "FTV_Cache" WHERE ' . $where,
                    'flush'   => 'DELETE FROM "FTV_Cache" WHERE "namespace" = :namespace',
                    'flushr'  => 'DELETE FROM "FTV_Cache" WHERE "namespace" = :namespace OR "namespace" LIKE :likens',
                );

                if (!isset($this->stmts[$key])) {
                    if (!isset($queries[$key])) {
                        throw new FTV_Exception('Cannot find query for key '.$key.'.');
                    }
