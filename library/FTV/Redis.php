     ...
     class FTV_Redis 
    {
        ...
        public static function db($name = 'default')
        {
            if (!isset(self::$databases[$name])) {
                if (!is_null(config::get("database.redis.$name"))) {
                    throw new FTV_Exception("Redis database [$name] is not defined.");
                }
                extract(config::get('database.redis'));
                self::$databases[$name] = new self($host, $port, $database);
            }
        ...
        protected function parse($response)
        {
            switch (substr($response, 0, 1)) {
                case '-':
                    throw new FTV_Exception('Redis error: ' . substr(trim($response), 4));
                ...
                default:
                    throw new FTV_Exception("Unknown Redis response: " . substr($response, 0, 1));
            }
        }
        ...
        protected function connect()
        {
            ...
            if ($this->connection === false) {
                throw new FTV_Exception("Error making Redis connection: {$error} - {$message}");
            }
        ...
        {
            return self::db()->run($method, $parameters);
        }
