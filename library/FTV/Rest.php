<?php 
    ...
    class FTV_Rest
    {
        ...
        private function processRequest()
        {
            ...
            unset($_GET['RESTurl']);
            $this->request['method'] = i::lower($_SERVER['REQUEST_METHOD']);
        ...
        public function process()
        {
            try {           
                ...
                if(null == $controllerName) {
                    throw new FTV_Exception('Method not allowed', 405);
                }       
                $controller = new ReflectionClass($controllerName);
                if(!$controller->isInstantiable()) {
                    throw new FTV_Exception('Bad Request', 400);
                }
                try {
                    $method = $controller->getMethod($this->request['method']);
                } catch(ReflectionException $re) {
                    throw new FTV_Exception('Unsupported HTTP method ' . $this->request['method'], 405);
                }
                if(!$method->isStatic()) {
                    $controller = $controller->newInstance($this->request);
                    if(!$controller->checkAuth()) {
                        throw new FTV_Exception('Unauthorized', 401);
                    }
                    $method->invoke($controller);
                    $this->response = $controller->getResponse();
                    $this->responseStatus = $controller->getResponseStatus();
                } else {
                    throw new FTV_Exception('Static methods not supported in Controllers', 500);
                }
                if(is_null($this->response)) {
                    throw new FTV_Exception('Method not allowed', 405);
                }
            } catch (FTV_Exception $re) {
        ...
        private function getHeaders()
        {
            ...
            foreach($keys as $val) {
                    $key = repl(' ', '-', ucwords(i::lower(repl('_', ' ', substr($val, 5)))));
